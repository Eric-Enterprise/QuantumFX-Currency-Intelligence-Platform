...
def number(raw):
    ...
    if len(raw) > 24 or not re.fullmatch(r"\d+(?:[.,]\d{1,8})?", raw):
        raise ValueError("Zahl ohne Tausendertrennzeichen eingeben, z. B. 1234,56 (max. 8 Nachkommastellen).")
    try:
        value = Decimal(raw.replace(",", "."))
    except InvalidOperation as exc:
        raise ValueError("Ungültige Zahl.") from exc
    if not value.is_finite() or value > Decimal("1000000000000"):
        raise ValueError("Betrag darf höchstens 1.000.000.000.000 sein.")
    return value


def validate_rates(rates):
    if not isinstance(rates, dict) or not rates:
        raise ValueError("Keine gültigen Kursdaten.")
    valid = {}
    for code, value in rates.items():
        ...
        try:
            ...
            raise ValueError("Ungültiger Kurs.") from exc
...
def cached_snapshot(data):
    if not isinstance(data, dict) or data.get("base") != "EUR":
        raise ValueError("Ungültiger Cache.")
    ...
    if day > date.today() or fetched.tzinfo is None or fetched > datetime.now(timezone.utc) + timedelta(minutes=5):
        raise ValueError("Ungültiges Cache-Datum.")
    rates = validate_rates(data["rates"])
    if rates.get("EUR") != 1:
        raise ValueError("Ungültige Basiswährung.")
    ...
    def latest(self):
        try:
            ...
            if data.get("base") != "EUR":
                raise ValueError("Falsche Basiswährung.")
            day = date.fromisoformat(data["date"])
            if day > date.today():
                raise ValueError("Kursdatum liegt in der Zukunft.")
            rates = validate_rates(data["rates"])
            rates["EUR"] = Decimal(1)
            result = Snapshot(rates, day.isoformat(), datetime.now(timezone.utc).isoformat(), "online")
        except Exception as exc:
            result = self.cached()
            result.warning = "Abruf fehlgeschlagen: " + str(exc)[:180]
            return result
        try:
            atomic_json(self.folder / "rates.json", result.payload())
        except OSError:
            result.warning = "Kurse geladen; lokaler Cache konnte nicht gespeichert werden."
        return result

    def history(self, base, target, days):
        if not all(re.fullmatch("[A-Z]{3}", x) for x in (base, target)) or days not in (30, 90, 365):
            raise ValueError("Ungültiger Zeitraum oder Währung.")
        end = date.today()
        ...
        def parse(data):
            if not isinstance(data, dict) or data.get("base") != base:
                raise ValueError("Ungültige Zeitreihe.")
            points = []
            ...
            if not points:
                raise ValueError("Keine Kurse im gewählten Zeitraum verfügbar.")
        ...
        try:
            ...
            try:
                ...
                raise ValueError("Kursverlauf nicht verfügbar. Internetverbindung prüfen und erneut laden.")
        try:
...
def convert(amount, base, target, rates, percent=Decimal(0), fixed=Decimal(0)):
    for value in (amount, percent, fixed):
        if not value.is_finite() or value < 0:
            raise ValueError("Negative oder ungültige Beträge sind nicht erlaubt.")
    if percent > 100:
        raise ValueError("Prozentuale Gebühr muss zwischen 0 und 100 liegen.")
    if base not in rates or target not in rates:
        raise ValueError("Währung in diesem Kursstand nicht verfügbar.")
    fee = amount * percent / 100 + fixed
    if fee > amount:
        raise ValueError("Die Gebühren übersteigen den Ausgangsbetrag.")
...
def money(value, currency):
    ...
    return f"{rounded:,.{digits}f}".replace(",", "_").replace(".", ",").replace("_", ".")
